...
| `--timeout SECONDS` | `CLAUDE_BRIDGE_TIMEOUT` | hard cap: kill a Claude run (and everything it spawned) after this long, busy or not (default 7200; 0 = none). Hermes' `gateway_timeout` is an *inactivity* limit that the stats line keeps alive, so this is the only wall-clock cap. If Claude had already answered and was only waiting on background work, the answer is kept and a note says what was stopped |
| `--bg-wait SECONDS` | `CLAUDE_BRIDGE_BG_WAIT` | after Claude has answered, how long `claude -p` may keep running for unfinished background work (subagents, background Bash) before it stops that work; the thread takes no new message meanwhile. Sets `CLAUDE_CODE_PRINT_BG_WAIT_CEILING_MS` (yours wins), never above `--timeout`. Default 600; 0 = up to `--timeout` |
| `--idle-timeout SECONDS` | `CLAUDE_BRIDGE_IDLE_TIMEOUT` | stop a run that has stalled: no stream output from Claude for this long while no tool is running, or — while one is — no CPU or I/O by any process Claude spawned (Claude's own tool heartbeats do not count). A busy build or rsync runs on to the hard cap. Default 600; 0 = off |
| `--stats-interval SECONDS` | `CLAUDE_BRIDGE_STATS_INTERVAL` | every N seconds of a run, post a line with CPU, memory and I/O for Claude and its tool processes, and which tools are running, next to the tool headlines (skipped while Claude is streaming text). Default 60; 0 = off |
...
| `--flush-interval N` | `CLAUDE_BRIDGE_FLUSH_INTERVAL` | with the Hermes plugin, on platforms where Hermes does not stream (e.g. Mattermost with `streaming.enabled: false`): post what Claude has done so far (its text and tool calls) every N seconds while it works, then the answer as its own message. Default 30; 0 = progress only at the end. |
